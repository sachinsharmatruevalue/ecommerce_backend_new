...
exports.getAllReportByUser = async (req, res) => {
  try {
    const userId =  req.user._id; // If userId is passed in params, use it; otherwise, use logged-in user's ID

    const reports = await UserReport.find({ user: userId }).sort({ createdAt: -1 });
